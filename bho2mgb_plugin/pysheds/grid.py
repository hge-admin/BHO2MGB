...
import pandas as pd
# import geojson
from .affine import Affine # INSIDE LIBRARY
...
from .view import BaseViewFinder, RegularViewFinder, IrregularViewFinder
from .view import RegularGridViewer, IrregularGridViewer
...
class Grid(object):
    ...
    def read_raster(self, data, data_name, band=1, window=None, window_crs=None,
        ...
        # read raster file
        f = gdal.Open(data)
        fb = f.GetRasterBand(band)
        shape = (fb.YSize, fb.XSize)
        affine = Affine.from_gdal(*f.GetGeoTransform())
        data = np.ma.filled(fb.ReadAsArray()).astype(float)
        nodata = fb.GetNoDataValue()
        
        try:
            crs = pyproj.Proj(f.GetProjection(), preserve_units=True)
        except:
            crs = self.crs
    ...
    def resize(self, data, new_shape, out_suffix='_resized', inplace=True,
               nodata_in=None, nodata_out=np.nan, apply_mask=True, ignore_metadata=True, **kwargs):
        """
    ...
    def catchment(self, x, y, data, pour_value=None, out_name='catch', dirmap=None,
                  ...
                  recursionlimit=15000, inplace=True, apply_mask=True, ignore_metadata=False,
                  **kwargs):
        """
    ...
    #     selfdf = pd.DataFrame(
    #         np.arange(self.view('dir', apply_mask=True).size).reshape(self.shape),
    ...
    def accumulation(self, data, weights=None, dirmap=None, nodata_in=None, nodata_out=0, efficiency=None,
                     out_name='acc', routing='d8', inplace=True, pad=False, apply_mask=True,
...
-                 be same size as data.
    ...
    def compute_hand(self, fdir, dem, drainage_mask, out_name='hand', dirmap=None,
                     ...
                     inplace=True, apply_mask=True, ignore_metadata=False, return_index=False,
                     **kwargs):
        """
    ...
    def compute_ltnd(self, fdir, cdist, drainage_mask, out_name='ltnd', dirmap=None,
                     nodata_in_fdir=None, nodata_in_cdist=None, nodata_out=np.nan, routing='d8',
                     inplace=True, apply_mask=True, ignore_metadata=False, return_index=False,
                     **kwargs):
        """
        Computes the length to nearest drainage (LTND), based on a flow direction grid,
        a cell distances grid, and a grid containing the locations of drainage channels.
        ...
        nodata_in_fdir : int or float
                         Value to indicate nodata in flow direction input array.
        nodata_in_cdist : int or float
                        Value to indicate nodata in cell distances input array.
        nodata_out : int or float
                     Value to indicate nodata in output array.
        routing : str
                  Routing algorithm to use:
        ...
        dirmap = self._set_dirmap(dirmap, fdir)
        nodata_in_fdir = self._check_nodata_in(fdir, nodata_in_fdir)
        nodata_in_cdist = self._check_nodata_in(cdist, nodata_in_cdist)
        properties = {'nodata' : nodata_out}
        # TODO: This will overwrite metadata if provided
        metadata = {'dirmap' : dirmap}
        # initialize array to collect catchment cells
        fdir = self._input_handler(fdir, apply_mask=apply_mask, nodata_view=nodata_in_fdir,
                                   properties=properties, ignore_metadata=ignore_metadata,
                                   **kwargs)
        cdist = self._input_handler(cdist, apply_mask=apply_mask, nodata_view=nodata_in_cdist,
                                  properties=properties, ignore_metadata=ignore_metadata,
                                  **kwargs)
        mask = self._input_handler(drainage_mask, apply_mask=apply_mask, nodata_view=0,
                                   properties=properties, ignore_metadata=ignore_metadata,
                                   **kwargs)
        assert (np.asarray(cdist.shape) == np.asarray(fdir.shape)).all()
        assert (np.asarray(cdist.shape) == np.asarray(mask.shape)).all()
        if routing.lower() == 'dinf':
            ...
            try:
                ...
                r_dirmap = np.array(dirmap)[[4, 5, 6, 7, 0, 1, 2, 3]].tolist()
                source = np.flatnonzero(mask)
                ltnd = -np.ones(fdir.size, dtype=float)
                ltnd[source] = 0
                for _ in range(fdir.size):
                    selection = self._select_surround_ravel(source, fdir.shape)
                    ix = (fdir.flat[selection] == r_dirmap) & (ltnd.flat[selection] < 0)
                    # TODO: Not optimized (a lot of copying here)
                    parent = np.tile(source, (len(dirmap), 1)).T[ix]
                    child = selection[ix]
                    dist = cdist.flat[child]
                    if not child.size:
                        break
                    ltnd[child] = ltnd[parent] + dist
                    source = child
                ltnd = ltnd.reshape(fdir.shape)
                if not return_index:
                    ltnd = np.where(ltnd != -1, ltnd, nodata_out)
                    ltnd = ltnd / 1000 # convert to km
            except:
                raise
            finally:
                mask = mask.reshape(fdir.shape)
                self._replace_rim(fdir, dirleft, dirright, dirtop, dirbottom)
                self._replace_rim(mask, maskleft, maskright, masktop, maskbottom)
            return self._output_handler(data=ltnd, out_name=out_name, properties=properties,
                                        inplace=inplace, metadata=metadata)

    def compute_hand_strbrn(self, fdem, dem, drainage_mask, edges_mask, out_name='hand', dirmap=None,
                     nodata_in_fdem=None, nodata_in_dem=None, nodata_out=np.nan, routing='d8',
                     inplace=True, apply_mask=True, ignore_metadata=False, return_index=False,
                     **kwargs):
        """
        Computes the height above nearest drainage (HAND), based on a filled elevation grid,
        a digital elevation grid, and a grid containing the locations of drainage channels.
        ...
        dirmap : list or tuple (length 8)
                 ...
                 cardinal and intercardinal directions (in order):
                 [N, NE, E, SE, S, SW, W, NW]
        nodata_in_fdem : int or float
                         Value to indicate nodata in filled dem input array.
        nodata_in_dem : int or float
                        Value to indicate nodata in digital elevation input array.
        nodata_out : int or float
                     Value to indicate nodata in output array.
        ...
        # TODO: Why does this use set_dirmap but flowdir doesn't?
        # dirmap = self._set_dirmap(dirmap, fdir)
        nodata_in_fdem = self._check_nodata_in(fdem, nodata_in_fdem)
        nodata_in_dem = self._check_nodata_in(dem, nodata_in_dem)
        properties = {'nodata' : nodata_out}
        # TODO: This will overwrite metadata if provided
        metadata = {'dirmap' : dirmap}
        ...
        mask = self._input_handler(drainage_mask, apply_mask=False, nodata_view=0,
                                   properties=properties, ignore_metadata=ignore_metadata,
                                   **kwargs)
        edges = self._input_handler(edges_mask, apply_mask=False, nodata_view=0,
                                   properties=properties, ignore_metadata=ignore_metadata,
                                   **kwargs)
        ...
        if routing.lower() == 'd8':
            try:
                ...
                for _ in range(ddsum):
                    ...
                    for n, i in enumerate(ix.T):
                        child = selection.T[n][i] # neighbour at surround direction i
                        selection_ix = self._select_surround_ravel(child, fdem.shape) # neighbourhood of neighbours
                        gradient = np.where((hand.flat[selection_ix]>=0)&(edges.flat[selection_ix]==0), fdem.flat[selection_ix], 9999)
                        ixx = np.nanargmin(gradient, axis=1)
                        parent = selection_ix[range(ixx.size), ixx]
                        hand[child] = hand[parent]
                hand = hand.reshape(dem.shape)
                if not return_index:
                    hand = np.where(hand != -1, dem - dem.flat[hand], nodata_out)
                ...
                mask = mask.reshape(dem.shape)
                self._replace_rim(fdem, fdemleft, fdemright, fdemtop, fdembottom)
                self._replace_rim(dem, demleft, demright, demtop, dembottom)
                self._replace_rim(mask, maskleft, maskright, masktop, maskbottom)
            return self._output_handler(data=hand, out_name=out_name, properties=properties,
                                        inplace=inplace, metadata=metadata)

    def compute_ltnd_strbrn(self, fdem, drainage_mask, edges_mask, out_name='ltnd', dirmap=None,
                     nodata_in_fdem=None, nodata_out=np.nan, routing='d8',
                     inplace=True, apply_mask=True, ignore_metadata=False, return_index=False,
                     **kwargs):
        """
        Computes the height above nearest drainage (HAND), based on a filled elevation grid,
        ...
        if routing.lower() == 'd8':
            try:
                fdemleft, fdemright, fdemtop, fdembottom = self._pop_rim(fdem, nodata=nodata_in_fdem)
                maskleft, maskright, masktop, maskbottom = self._pop_rim(mask, nodata=0)
                mask = mask.ravel()
                source = np.flatnonzero(mask)
                ...
                for _ in range(ddsum):
                    ...
                    for n, i in enumerate(ix.T):
                        ...
                        child = selection.T[n][i] # neighbour at surround direction i
                        selection_ix = self._select_surround_ravel(child, fdem.shape) # neighbourhood of neighbours
                        gradient = np.where((ltnd.flat[selection_ix]>=0)&(edges.flat[selection_ix]==0), fdem.flat[selection_ix], 9999)
                        ixx = np.nanargmin(gradient, axis=1)
                        parent = selection_ix[range(ixx.size), ixx]
                        distances = dist.flat[child]
                        ltnd[child] = ltnd[parent] + distances
                ltnd = ltnd.reshape(fdem.shape)
                if not return_index:
                    ltnd = np.where(ltnd != -1, ltnd, nodata_out)
                    ltnd = ltnd / 1000 # convert to km
            except:
    ...
    def _convert_grid_indices_crs(self, grid_indices, old_crs, new_crs):
        if _OLD_PYPROJ:
            ...
            transformer = pyproj.Transformer.from_proj(old_crs, new_crs, always_xy=True)
            x2, y2 = transformer.transform(grid_indices[:,1], grid_indices[:,0], errcheck=True)
    ...
    def to_ascii(self, data_name, file_name, view=True, delimiter=' ', fmt=None,
                 apply_mask=True, nodata=None, interpolation='nearest',
    ...
    def to_raster(self, data_name, file_name, profile=None, view=True, blockxsize=256,
                  blockysize=256, apply_mask=True, nodata=None, interpolation='nearest',
        ...
        outdata = out_driver.Create(file_name,
                                       width,
                                       height,
                                       1,
                                       gdal_dtype)
        
        outdata.GetRasterBand(1).WriteArray(np.asarray(data))
        outdata.GetRasterBand(1).SetNoDataValue(float(data.nodata))
        outdata.SetGeoTransform(Affine.to_gdal(data.affine))
        outdata.SetProjection(data.crs.srs)
        outdata.FlushCache()
    ...
    def detect_pits(self, data, nodata_in=None, apply_mask=True, ignore_metadata=True,
                    **kwargs):
        """
    ...
    def detect_flats(self, data, nodata_in=None, apply_mask=True, ignore_metadata=True, **kwargs):
        """
    ...
    def detect_cycles(self, fdir, max_cycle_len=50, dirmap=None, nodata_in=0, nodata_out=-1,
        ...
        fdir = self._input_handler(fdir, apply_mask=apply_mask, nodata_view=nodata_in,
                                   ...
                                   ignore_metadata=ignore_metadata, **kwargs)
        if np.isnan(nodata_in):
            in_catch = np.isfinite(fdir.ravel())
        else:
    ...
    def fill_pits(self, data, out_name='filled_dem', nodata_in=None, nodata_out=0,
                  inplace=True, apply_mask=True, ignore_metadata=False, **kwargs):
        """
    ...
    def resolve_flats(self, data=None, out_name='inflated_dem', nodata_in=None, nodata_out=None,
        ...
        drainage_grad = drainage_grad.astype(float)
        ...
        dem_out = dem.astype(float) + drainage_grad
        return self._output_handler(data=dem_out, out_name=out_name, properties=grid_props,
                                    inplace=inplace, metadata=metadata)
    ...
    def fill_depressions(self, data, out_name='flooded_dem', nodata_in=None, nodata_out=None,
        ...
        dem_mask[:, -1] = True
        # Make sure nothing flows to the nodata cells
        nanmax = dem[np.isfinite(dem)].max()
        seed = np.copy(dem)
    ...
    def raise_nondraining_flats(self, data, out_name='raised_dem', nodata_in=None,
                                nodata_out=np.nan, inplace=True, apply_mask=True,
                                ignore_metadata=False, **kwargs):
        """
    ...
    def detect_depressions(self, data, nodata_in=None, nodata_out=np.nan,
                           inplace=True, apply_mask=True, ignore_metadata=False,
                           **kwargs):
        """
    ...
    def rasterize(self, shapes, out_shape=None,
                  fill=0, out=None, transform=None,
                  all_touched=False, default_value=1, dtype=None, out_name='rasterized', 
                  nodata_in=None, nodata_out=None, inplace=True, apply_mask=False, 
                  ignore_metadata=False, **kwargs):
        """
        ...
        Parameters
        ----------
        shapes : file path with geometries
                    linestrings or polygons (if latter, rasterize edges)**
                    changed for MGB-BHO applications, not yet generalized!!!
        ...
        out_name : string
                    Name of attribute containing new rasterized array.
        nodata_in : int or float
                    Value to indicate nodata in input array.
        nodata_out : int or float
                    Value indicating no data in output array.
...
#        cellsize = self.cellsize
        ...
        gdal_dtype = NP2GDAL_CONVERSION['uint8']
        target_ds = gdal.GetDriverByName('MEM').Create('',
                                                 int(out_shape[1]),
                                                 int(out_shape[0]),
                                                 1,
                                                 gdal_dtype)
        target_ds.SetGeoTransform(Affine.to_gdal(transform))
        ...
        gdal.RasterizeLayer(target_ds,
                                  [1],
                                  lyr,
                                  burn_values=[default_value],
                                  options =["ALL_TOUCHED=TRUE"])
        raster = target_ds.GetRasterBand(1).ReadAsArray()
        
        return self._output_handler(data=raster, out_name=out_name, properties=grid_props,
                            inplace=inplace, metadata=metadata)
    ...
    # def burn_dem(self, dem, streams, edges, out_name='burned_dem', burn_value=-100, sharp_value=50,
    #               nodata_in=None, nodata_out=None,
    #                       inplace=True, apply_mask=True, ignore_metadata=False, **kwargs):
    #     """
    #     Burn values from shape (streamline or edges) into a raster (dem)
       
    #     Parameters
    #     ----------
    #     shapes : SHP file (line or polygon)
    #                 If polygon, converted to outlines (edges)
    #     dem : str or Raster
    #            DEM data.
    #                If str: name of the dataset to be viewed.
    #                If Raster: a Raster instance (see pysheds.view.Raster)
    #     streams and edges : str or Raster
    #            Mask data to be burned into DEM.
    #                If str: name of the dataset to be viewed.
    #                If Raster: a Raster instance (see pysheds.view.Raster)
    #     burn_value : int
    #                 Value which lines will be replaced in dem (negative for stream burn and positive for edge sharp)
    #     out_name : string
    #                 Name of attribute containing new burned dem array.
    #     nodata_in : int or float
    #                 Value to indicate nodata in input array.
    #     nodata_out : int or float
    #                 Value indicating no data in output array.
    ...
    #                               properties=grid_props, ignore_metadata=ignore_metadata,
    #                               **kwargs)
    #     streams = self._input_handler(streams, apply_mask=apply_mask, nodata_view=nodata_in,
    #                               properties=grid_props, ignore_metadata=ignore_metadata,
    #                               **kwargs)        
    #     edges = self._input_handler(edges, apply_mask=apply_mask, nodata_view=nodata_in,
    #                               properties=grid_props, ignore_metadata=ignore_metadata,
    #                               **kwargs)             
    #     edges[streams==1] = 0
    #     edges = edges * sharp_value
    #     streams = streams * burn_value
    #     burned_dem = np.asarray(dem)
    #     burned_dem = dem + streams + edges
    #     return self._output_handler(data=burned_dem, out_name=out_name, properties=grid_props,
    #                         inplace=inplace, metadata=metadata)

    def burn_dem(self, dem, streams, out_name='burned_dem',
                sharp_value=80, buff_value=4, smooth_value=8,
                nodata_in=None, nodata_out=None, out_shape=None, transform=None,
                        inplace=True, apply_mask=True, ignore_metadata=False, **kwargs):
        """
        Burn values from shape (streamline or edges) into a raster (dem)
        ...
        dem = self._input_handler(dem, apply_mask=apply_mask, nodata_view=nodata_in,
                                  properties=grid_props, ignore_metadata=ignore_metadata,
                                  **kwargs)
        streams = self._input_handler(streams, apply_mask=False, nodata_view=nodata_in,
                                  properties=grid_props, ignore_metadata=ignore_metadata,
                                  **kwargs)        
        
        # Takes DEM values in stream line (vectallo)
        #dem = dem * 1000 # Increase slope gradients (better for filling)
        # elv_strl = streams * dem
        
        # Calculates distances from hydrography (vectdist)
        # Data source
        gdal_dtype = NP2GDAL_CONVERSION['int32']
        ...
        ds = gdal.GetDriverByName('MEM').Create('',
                                                 ...
                                                 3,
                                                 gdal_dtype)
        ds.SetGeoTransform(Affine.to_gdal(transform))
        
        streams_band = ds.GetRasterBand(1)
        strdist_band = ds.GetRasterBand(2)
        bufgrid_band = ds.GetRasterBand(3)
        
        # streams
        streams_band.WriteArray(streams)
        
        # distance from streams
        gdal.ComputeProximity(streams_band, strdist_band, ["VALUES=1","DISTUNITS=PIXEL", "MAXDIST="+str(buff_value+1), "NODATA=0"])
        gdal.ComputeProximity(streams_band, bufgrid_band, ["VALUES=1","DISTUNITS=PIXEL", "MAXDIST="+str(buff_value),"FIXED_BUF_VAL=1",  "NODATA=0"])
        
        strdist = smooth_value * (strdist_band.ReadAsArray() - buff_value)
        bufgrid = bufgrid_band.ReadAsArray()
        
        agree_buffer = strdist * bufgrid
        agree_sharp = - sharp_value * streams 
        agree_stream = agree_buffer + agree_sharp
        agreedem = dem + agree_stream
        
        return self._output_handler(data=agreedem, out_name=out_name, properties=grid_props,
                            inplace=inplace, metadata=metadata)
